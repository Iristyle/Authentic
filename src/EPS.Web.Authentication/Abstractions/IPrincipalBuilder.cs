...
namespace EPS.Web.Abstractions
{
	...
	public interface IPrincipalBuilder
	{
		...
		/// 								be null. </param>
		/// <param name="credential">	 	The username and password that have been extracted from the request. </param>
		/// <returns>	An IPrincipal if the given credentials could be authenticated, otherwise null. </returns>
		IPrincipal ConstructPrincipal(HttpContextBase context, MembershipUser membershipUser, NetworkCredential credential);
	}
}