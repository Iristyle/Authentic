...
namespace EPS.Web.Handlers.Tests.Unit
{
    public class FileHttpHandlerTest : IDisposable
    {
        ...
        {
            Assert.Throws<ArgumentNullException>(() => new FileHttpHandler(container.Resolve<IFileHttpHandlerConfiguration>(), null, container.Resolve<IFileHttpHandlerStatusLog>()));
        }

        [Fact]
        public void FileHttpHandler_ThrowsOnNullLogger()
        {
            Assert.Throws<ArgumentNullException>(() => new FileHttpHandler(container.Resolve<IFileHttpHandlerConfiguration>(), container.Resolve<IFileHttpHandlerStreamLoader>(), null));
        }
